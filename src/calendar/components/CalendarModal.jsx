import { useState, useEffect } from "react";
import { addHours } from "date-fns";
import Modal from "react-modal"
import DatePicker, {registerLocale} from "react-datepicker";
import "react-datepicker/dist/react-datepicker.css";
import { es } from "date-fns/locale";
import { differenceInMilliseconds } from "date-fns/esm";
import Swal from "sweetalert2";
import { useMemo } from "react";
import { useUiStore, useCalendarStore } from "../../hooks";
import { useSelector } from "react-redux";
// import 'sweetalert2/dist/sweetalert2.min.css'

registerLocale('es', es)

const customStyles = {
    content: {
        top: '50%',
        left: '50%',
        right: 'auto',
        bottom: 'auto',
        marginRight: '-50%',
        transform: 'translate(-50%, -50%)',
    },
};

Modal.setAppElement('#root');

export const CalendarModal = () => {

    const { isDateModalOpen, closeDateModal } = useUiStore();

    const { activeEvent, startSavingEvent } = useCalendarStore();

    const [ formSubmitted, setFormSubmitted ] = useState(false);

    const [formValues, setFormValues] = useState({
        title : '',
        notes: '',
        start: new Date(),
        end: addHours( new Date(), 1)
    });

    const titleClass = useMemo(() => {
        if(!formSubmitted) return '';
        return (formValues.title.length === 0)
        ?  'is-invalid'
        :   'is-valid' 
    } ,[formValues.title, formSubmitted])


    useEffect(() => {
      if(activeEvent !== null ){
        setFormValues({...activeEvent})
      }
    }, [activeEvent])
    

    const onInputChanged = ({target}) =>{
        setFormValues({
            ...formValues,
            [target.name]:target.value
        })

    }

    const onDateChanged = (event, changing) =>{
        // console.log(event, changing);
        setFormValues({
            ...formValues,
            [changing]:event
        })
    }

    const onSubmit = async (event) =>{
        setFormSubmitted(true);
        console.log(formValues)
        event.preventDefault()
        const difference = differenceInMilliseconds(formValues.end, formValues.start);
        
        if((difference < 0) || Number.isNaN(difference)){
            Swal.fire('Error', 'Must provide two dates','error')
            console.log('Error on date.');
            return;
        }

        if(formValues.title.length === 0){
            console.log('Title must be provided.');
            Swal.fire('Error', 'Must provide the title','error')
            return;
        }

        Swal.fire('Succes!','The new evet was added.','success')
        console.log('FormValue =====>>>',formValues)
        await startSavingEvent(formValues)
        closeDateModal();
        setFormSubmitted(false)
        console.log(difference)
    }
    const onCloseModal = () => {
        // setOpen(false);
        console.log('Closing modal')
        closeDateModal();
    }

    return (
        <Modal
            isOpen={isDateModalOpen}
            onRequestClose={onCloseModal}
            style={customStyles}
            className='modal'
            overlayClassName='modal-fondo'
            closeTimeoutMS={200}
        >
            <h1> Nuevo evento </h1>
            <hr />
            <form className="container" onSubmit={onSubmit}>

                <div className="form-group mb-2">
                    <label>Fecha y hora inicio</label>
                    <DatePicker
                        selected={formValues.start}
                        name="start"
                        className='form-control'
                        onChange={(event)=> onDateChanged(event,'start')}
                        dateFormat="Pp"
                        showTimeSelect
                        locale="es"
                        timeCaption="Hora"
                    />
                    {/* <input className="form-control" onChange={onInputChanged} value={formValues.start} placeholder="Fecha inicio" /> */}
                </div>

                <div className="form-group mb-2">
                    <label>Fecha y hora fin</label>
                    <DatePicker
                        minDate={formValues.start}
                        selected={formValues.end}
                        name="end"
                        className='form-control'
                        onChange={(event)=> onDateChanged(event,'end')}
                        dateFormat="Pp"
                        showTimeSelect
                        locale="es"
                        timeCaption="Hora"
                    />
                    {/* <input className="form-control" onChange={onInputChanged} value={formValues.end} placeholder="Fecha inicio" /> */}
                </div>

                <hr />
                <div className="form-group mb-2">
                    <label>Titulo y notas</label>
                    <input
                        type="text"
                        className={`form-control ${titleClass}`}
                        placeholder="Título del evento"
                        value={formValues.title}
                        name="title"
                        onChange={onInputChanged}
                        autoComplete="off"
                    />
                    <small id="emailHelp" className="form-text text-muted">Una descripción corta</small>
                </div>

                <div className="form-group mb-2">
                    <textarea
                        type="text"
                        className="form-control"
                        placeholder="Notas"
                        value={formValues.notes}
                        rows="5"
                        name="notes"
                        onChange={onInputChanged}
                    ></textarea>
                    <small id="emailHelp" className="form-text text-muted">Información adicional</small>
                </div>

                <button
                    type="submit"
                    className="btn btn-outline-primary btn-block"
                >
                    <i className="far fa-save"></i>
                    <span> Guardar</span>
                </button>

            </form>
        </Modal>
    )
}
